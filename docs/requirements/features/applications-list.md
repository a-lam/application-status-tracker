...
| # | Given | When | Then |
|---|-------|------|------|
| AC-12-1 | An application exists | The page renders | Each application card displays: a top row containing a title-group (Job Title, Employer italicised directly below, Status badge below Employer) on the left, Due Date immediately left of the kebab on the right; a visual separator between the header block and the metadata block; then: Job Listing URL link (if present), Job Start (if present), Salary (if present, prefixed with a "SALARY:" label), Job Description, and a collapsible Artifacts panel |
...
| AC-12-8 | An application's status is `NOT_SUBMITTED` | The page renders | A grey status badge reading "Not Submitted" is shown below the employer line |
| AC-12-9 | An application's status is `SUBMITTED` | The page renders | A blue status badge reading "Submitted" is shown below the employer line |
| AC-12-10 | An application's status is `INTERVIEWING` | The page renders | A purple status badge reading "Interviewing" is shown below the employer line |
| AC-12-11 | An application's status is `OFFER_RECEIVED` | The page renders | An amber status badge reading "Offer Received" is shown below the employer line |
| AC-12-12 | An application's status is `OFFER_ACCEPTED` | The page renders | A bold green status badge reading "Offer Accepted" is shown below the employer line |
| AC-12-13 | An application's status is `OFFER_DECLINED` | The page renders | An orange status badge reading "Offer Declined" is shown below the employer line |
| AC-12-14 | An application's status is `REJECTED` | The page renders | A red status badge reading "Rejected" is shown below the employer line |
| AC-12-15 | An application's status is `WITHDRAWN` | The page renders | A grey status badge reading "Withdrawn" is shown below the employer line |
...
| FR-APPS-27 | The card top row contains a title-group (flex column: Job Title on top, Employer italicised directly below, Status badge below Employer) that fills available width, with the `Due:` date immediately to its right and the kebab menu rightmost. This grouping ensures Employer is always visually adjacent to Job Title regardless of the kebab's touch-target height. Job Start is displayed below the "View Job Listing →" link (or below the top row if no listing link is present). A specific date (`jobStartDate`) is formatted as `D Mon YYYY` (e.g. `1 Sep 2026`). Approximate text (`jobStartText`) is shown as-is. The display label **"JOB START:"** is rendered as a small, bold, uppercase inline label immediately followed by the value on the same line. If neither `jobStartDate` nor `jobStartText` is set, no Job Start line is rendered. |
| FR-APPS-28 | The application status must be displayed as a small pill/badge below the employer line inside the title-group — not inline with the job title. The badge colour must reflect the status: `NOT_SUBMITTED` → grey; `SUBMITTED` → blue; `INTERVIEWING` → purple; `OFFER_RECEIVED` → amber; `OFFER_ACCEPTED` → bold green; `OFFER_DECLINED` → orange; `REJECTED` → red; `WITHDRAWN` → grey. Dark-mode badge colours must use or extend the existing `.urgency-badge--*` CSS token set. |
| FR-APPS-29 | The card top row (`.app-card__row1`) must use `align-items: center` so the due date and kebab button align to the vertical midpoint of the title group, not to the top edge. |
| FR-APPS-30 | A visual separator (subtle `border-top` or equivalent `padding-top`) must divide the header block (job title, employer, status badge) from the metadata block (listing link, job start, salary) within each card. |
| FR-APPS-31 | Typography within application cards must meet the following minimum sizes: job title `1.0625rem` at `font-weight: 700`; due date `0.875rem`; inline metadata labels (SALARY:, JOB START:) `0.8125rem` bold uppercase. Description body text must use `color: var(--text)` (primary colour) rather than `var(--text-2)` to ensure readability against all urgency-band backgrounds. |
